...
class PoliciesController extends AppController
{
    ...
    public function add($policyTypeId='')
    {
        ...
        if ($this->request->is('post')) {
			...
			if(isset($postedData['policy_path']['name']) && !empty($postedData['policy_path']['name'])){	
				...
				///
				  $allowextension=array("jpeg","jpg","png","gif","pdf");
				if(in_array(strtolower($extFile),$allowextension))
                {
			...
			if($fileuplodStatus===false){
				...
				if($extAllow===true)
					...
					$this->Flash->error(__('File must be of following extension jpeg,jpg,png,gif,pdf'));
	...
	public function edit($id)
	{
		...
		$selectListdata	=	$this->PolicyTypes->find('list',[		
						'keyField' => 'id',
						'valueField' => 'policy_name'	
		])->toArray();
